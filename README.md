...
│   └── LaSRC_container_validation_example.ipynb
└── scripts/
...
The reproducible workflow notebook intentionally has cleared outputs so it remains
small and free of local paths or credential metadata. For a GitHub-rendered preview
of one panel and two summary figures, open
[`LaSRC_container_validation_example.ipynb`](notebooks/LaSRC_container_validation_example.ipynb).
It embeds only credential-free figures from a three-granule immediate-output smoke
test. Run `LaSRC_container_validation.ipynb` locally to generate current results.
